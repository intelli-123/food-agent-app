// company_rules.js
export const COMPANY_POLICIES = `
1. **Authenticity**: The image must realistically represent the food item claimed.
2. **Clarity**: The image must not be blurry, overly dark, or distorted.
3. **Purity**: The image must contain only the food item and relevant plating (no people, pets, or clutter).
4. **Safety**: Strictly NO nudity, violence, or prohibited substances.
5. **Consistency**: If the user claims "Veg", the image must not contain meat.
`;